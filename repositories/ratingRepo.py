...
def delete_rating(rating_id: str, db: Session = Depends(get_db), username: str = Depends(get_current_user)):
    ...
    if rating.user_id != user.id:
        raise HTTPException(status_code = 403, detail="You are not the user who made this review. Only the owner of the review can delete it.")
    return rating
...
def get_average(product_id: str, db: Session = Depends(get_db)):
    average = db.query(func.avg(RatingModel.rating).label('average')).filter(RatingModel.product_id==product_id).scalar()
