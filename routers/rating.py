...
from repositories.ratingRepo import create_rating as cr, delete_rating as dr, update_rating as ur, get_ratings as gr, get_average as ga
...
async def delete_rating(rating_id: str, db: Session = Depends(get_db), username: str = Depends(get_current_user)):
    ...
    """
    return JSONResponse(status_code=200, content=jsonable_encoder(dr(rating_id=rating_id, db=db, username=username).to_dict()))

#TODO: check if functional
@router.put("/rating", dependencies=[Depends(auth)])
async def update_rating(upd_rating: UpdateRating, db: Session = Depends(get_db), username: str = Depends(get_current_user)):
    """
    Update an existing rating
    """
    return JSONResponse(status_code=200, content=jsonable_encoder(ur(rating=upd_rating, db=db, username=username).to_dict()))
