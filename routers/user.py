...
from auth.user_auth import sign_up, check_email_auth, resend_email_code as rc, forgot_password as fp, \
    confirm_forgot_password as cfp, sign_in_auth
...
async def create_user(user: CreateUser, db: Session = Depends(get_db)):
    ...
    if len(user.password) < 8:
        ...
        raise HTTPException(status_code=500, detail="Password must have at least one lowercase letter")
            ...
            char in ['!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '-', '_', '+', '='] for char in user.password):
